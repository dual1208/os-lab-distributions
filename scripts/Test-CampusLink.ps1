...
& ssh gz 'set -eu; if ! command -v tcpdump >/dev/null; then export DEBIAN_FRONTEND=noninteractive; apt-get update -qq; apt-get install -y -qq tcpdump >/dev/null; fi; rm -f /run/campus-link/udp443.pcap; systemd-run --quiet --collect --unit=campus-link-capture --property=RuntimeMaxSec=25 /usr/bin/tcpdump -ni any udp port 443 -c 96 -w /run/campus-link/udp443.pcap'
